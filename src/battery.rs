...
mod tests {
    ...
    fn test_init_state_accepts_valid_values() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(50.0.kwh().expect("OK"), kw!(25.0));
        assert!(state.is_ok());
    }
    ...
    fn test_init_state_accepts_soc_at_capacity() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(100.0.kwh().expect("OK"), Power::zero());
        assert!(state.is_ok());
    }
    ...
    fn test_init_state_rejects_negative_soc() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state((-10.0).kwh().expect("OK"), Power::zero());
    ...
    fn test_init_state_rejects_soc_above_capacity() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(150.0.kwh().expect("OK"), Power::zero());
    ...
    fn test_init_state_rejects_power_above_max() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(50.0.kwh().expect("OK"), kw!(60.0));
    ...
    fn test_max_achievable_charge_power_limited_by_capacity() {
        ...
        let state = battery.init_state(95.0.kwh().expect("OK"), kw!(0.0)).expect("valid state");
    ...
    fn test_max_achievable_charge_power_limited_by_max_power() {
        ...
        let state = battery.init_state(Energy::zero(), kw!(0.0)).expect("valid state");
        let max_power = battery.max_achievable_charge_power(&state, hour!(1.0));
    ...
    fn test_max_achievable_discharge_power_limited_by_soc() {
        ...
        let state = battery.init_state(5.0.kwh().expect("OK"), kw!(0.0)).expect("valid state");
    ...
    fn test_max_achievable_discharge_power_limited_by_max_power() {
        ...
        let state = battery.init_state(100.0.kwh().expect("OK"), kw!(0.0)).expect("valid state");
        let max_power = battery.max_achievable_discharge_power(&state, hour!(1.0));
        assert_abs_diff_eq!(max_power.as_kw(), 50.0, epsilon = EPSILON);
    }
    ...
    fn test_charge_clamps_to_capacity() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(90.0.kwh().expect("OK"), kw!(0.0)).expect("valid state");
    ...
    fn test_discharge_normal_operation() {
        let battery = Battery::new(100.0.kwh().expect("OK"), kw!(50.0), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(50.0.kwh().expect("OK"), Power::zero()).expect("valid state");
    ...
    fn test_discharge_clamps_to_max_power() {
        let battery = Battery::new(100.0.kwh().expect("OK"), 50.0.kw(), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(100.0.kwh().expect("OK"), 0.0.kw()).expect("valid state");
        // Request 100 kW but max is 50 kW
    ...
    fn test_discharge_clamps_to_zero_soc() {
        let battery = Battery::new(100.0.kwh().expect("OK"), 50.0.kw(), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(5.0.kwh().expect("OK"), 0.0.kw()).expect("valid state");
    ...
    fn test_step_positive_power_charges() {
        let battery = Battery::new(100.0.kwh().expect("OK"), 50.0.kw(), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(50.0.kwh().expect("OK"), 0.0.kw()).expect("valid state");
    ...
    fn test_step_zero_power_maintains_soc() {
        let battery = Battery::new(100.0.kwh().expect("OK"), 50.0.kw(), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(50.0.kwh().expect("OK"), 10.0.kw()).expect("valid state");
    ...
    fn test_multiple_charge_cycles() {
        let battery = Battery::new(100.0.kwh().expect("OK"), 50.0.kw(), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(Energy::zero(), 0.0.kw()).expect("valid state");
    ...
    fn test_battery_state_accessors() {
        let battery = Battery::new(100.0.kwh().expect("OK"), 50.0.kw(), 0.81.fraction())
            .expect("battery should be valid");
        let state = battery.init_state(75.0.kwh().expect("OK"), 25.0.kw()).expect("valid state");
